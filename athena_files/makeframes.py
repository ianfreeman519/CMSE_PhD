...
if len(sys.argv) != 4:
    print("Usage: python makeframes.py <dictkey> <directoryname> <step>")
    sys.exit(1)
...
fileseries = grabFileSeries(directoryname + "/", 25000, basename=basename)
ts = yt.DatasetSeries(fileseries)
for index in np.arange(100, 25000, step=step, dtype=int):
    ...
    fig.savefig(f"figures/{dictkey}_x20_Xnull_{index//step:05}.png")
    plt.close(fig)
