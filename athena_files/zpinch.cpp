...
Real bphi0, bz;
Real eta_ohm;
Real phiperturb, phiperturbangle;
// Real nu_iso, eta_ohm;
} // namespace
...
void Mesh::InitUserMeshData(ParameterInput *pin) {
  ...
  phiperturb = pin->GetOrAddReal("problem", "phiperturb",0.0);
  phiperturbangle = pin->GetOrAddReal("problem", "phiperturbangle",0.0);
    ...
    for (int j=js; j<=je; j++) {
      for (int i=is; i<=ie; i++) {
        ...
        if (std::strcmp(COORDINATE_SYSTEM, "cylindrical") == 0) {
          ...
          phydro->u(IM1,k,j,i) = rho*vr*x1/rad * (1.0 + phiperturb*std::cos(2*phi + phiperturbangle));
          phydro->u(IM2,k,j,i) = rho*vr*x2/rad + (1.0 + phiperturb*std::cos(2*phi + phiperturbangle));
        }
