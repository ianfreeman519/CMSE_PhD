...
if len(sys.argv) != 5:
    print("Usage: python makeframes.py <dictkey> <directoryname> <startIndex> <stopIndex>")
    sys.exit(1)
...
fileseries = grabFileSeries(directoryname + "/", index, basename=basename)
...
if write_to_file == True:
    fname = f"energies_{zoom:.2f}x_with_flux_{dictkey}_s{startIndex}_e{stopIndex}.txt"
    f = open(fname, "a")
    f.write(h + "\n")
    print(f"writing to file " + fname, flush=True)
...
for i in range(startIndex, stopIndex):
    ...
    s = f"{i}\t,\t{tnow:.8e},\t{MEtot:.5e},\t{mechange:.5e}\t,\t{KEtot:.5e}\t,\t{kechange:.5e}\t,\t{TEtot:.5e}\t,\t{techange:.5e}\t,\t{Etot:.5e}"
    print(s, flush=True)
